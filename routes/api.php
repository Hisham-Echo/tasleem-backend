...
Route::prefix('v1')->middleware('auth:sanctum')->group(function () {
    ...
    // ── Cart ──────────────────────────────────────────────────────────
    // FIX 1: clear route MUST come before apiResource, otherwise
    // DELETE /cart/clear/{user_id} is swallowed by DELETE /cart/{cart}
    Route::delete('cart/clear/{user_id}', [CartItemController::class, 'clear']);
    Route::apiResource('cart', CartItemController::class);

    // ── Wishlist ──────────────────────────────────────────────────────
    // FIX 2: specific routes (check, clear) MUST come before /{id},
    // otherwise DELETE /wishlist/clear/{userId} matches /{id} with id=clear
    ...
    // FIX 3: delete-multiple MUST come before /{id},
    // otherwise DELETE /product-images/delete-multiple matches /{id} with id=delete-multiple
